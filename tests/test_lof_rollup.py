...
class TestHelpers(object):
    def test_get_gene_key(self):
        ...
        assert (
            lof_rollup.get_gene_key({"Gene": "foo", "SYMBOL": "bar"}, symbol_only=True)
            == "bar"
        )

    def test_create_epacts_group(self, ma_var):
        assert lof_rollup.create_epacts_group([{"var": ma_var, "allele_i": 1}]) == [
            "1:97078987_G/A"
        ]
        assert lof_rollup.create_epacts_group([{"var": ma_var, "allele_i": 2}]) == [
            "1:97078987_G/T"
        ]
...
class TestGetVarGeneData(object):
    def test_pass_filter(self, test_vep_vcf, ma_var):
        assert lof_rollup.get_var_gene_data(test_vep_vcf, ma_var, "True", []) == {
            "DPYD(ENSG00000188641)": [{"allele_i": 1, "var": ma_var}]
        }
    ...
    def test_symbol_only(self, test_vep_vcf, ma_var):
        assert lof_rollup.get_var_gene_data(
            test_vep_vcf, ma_var, "True", [], symbol_only=True
        ) == {"DPYD": [{"allele_i": 1, "var": ma_var}]}
...
class TestGetSampleLofCounts(object):
    def test_basic(self, test_vep_vcf):
        ...
        counts = lof_rollup.get_sample_lof_counts(
            list(gd.values())[0], test_vep_vcf.vcf.samples
        )
...
class TestMain(object):
    def test_lof(self, vcf_file):
        ...
        with open(fn, "r") as fh:
            ...
            assert sorted(list(filter(lambda x: data[x] != "0", data))) == [
                "HG00185",
                "HG00349",
                "HG00357",
            ]
    ...
    def test_freqs(self, vcf_file):
        ...
        with open(fn + "_freqs", "r") as fh:
            ...
            assert next(reader) == ["DPYD(ENSG00000188641)", "3", "382", str(3.0 / 382)]
