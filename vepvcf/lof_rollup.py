...
def main():
	parser = argparse.ArgumentParser(description="Generate LOF rollup file from VEP-annotated VCF file")
	...
	parser.add_argument('-f', '--filter', default='#PICK == "1" and #AF <= 0.05 and @lof', help="Filter string eval'd to select variants, use #[field_name] to denote VEP fields (default: \"%(default)s\")")
	...
	parser.add_argument('-g', '--group_file', help="Create EPACTS group file with this name")
	parser.add_argument('-q', '--freq_file', help="Create frequency output file with this name")
	parser.add_argument('-p', '--pops_file', help="Additionally use population assignments in this file for frequency calculations")
	parser.add_argument('-s', '--symbol_only', action="store_true", help="By default genes are shown as [gene_symbol]([ensembl_id]); use this flag to show only [gene_symbol]")
	...
	for vcf_file in args.vcf:
		...
		for var in vcf_iter:
			...
			if prev_chrom is not None and var.CHROM != prev_chrom:
				process_gene_data(gene_data, vep_vcf.vcf.samples, lof_table, epacts_groups, args.group_file)
	...
	with smart_open(args.output_file) as fh:
		...
		for s in samples:
			smart_write(fh, s + "\t" + "\t".join(map(lambda x: str(lof_table[x][s]), genes)) + "\n")
...
def create_epacts_group(gene_data):
	...
	for var_data in gene_data:
		...
		# [CHROM]:[POS]_[REF]/[ALT]
		group.append('{}:{}_{}/{}'.format(var.CHROM, var.POS, var.REF, alleles[var_data['allele_i']]))
