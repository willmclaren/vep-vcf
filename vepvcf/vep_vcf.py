...
for t in VEP_SO_TERMS.keys():
	FILTER_REPLACEMENTS['@' + t + '+'] = '#Consequence in [' + ','.join(
		list(map(
			lambda x: '"' + x + '"',
			filter(
...
def parse_filter_string(filter_string):
	...
	filter_fields = list(map(lambda x: x.replace('#', ''), re.findall(r'#[\w\-\_]+', filter_string)))
	...
	def vep_cols(self):
		if self._vep_cols is None:
			if self.vep_info_field in self.vcf:
				matches = re.search("Format: (.+?)\"", self.vcf[self.vep_info_field]['Description'])
				if matches:
		...
		if vep_data_string:
			for chunk in vep_data_string.split(','):
				...
				if 'ALLELE_NUM' in vep_data:
					...
					if (a in alleles and a in trimmed_alleles) or (a not in alleles and a not in trimmed_alleles):
						raise Exception("Unable to identify allele from VEP data - you can avoid this issue by running VEP with --allele_number")
		...
		for info_field in info_fields:
			...
			if values is not None:
				...
				if len(values) == 1:
					...
					raise Exception("Number of values for INFO field " + info_field + " does not match number of ALTs")
