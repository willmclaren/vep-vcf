...
def main():
    parser = argparse.ArgumentParser(
        description="Filter VCF file, optionally with VEP-annotations"
    )
    ...
    parser.add_argument(
        ...
        "--filter",
        default="1",
        help='Filter string eval\'d to select variants, use #[field_name] to denote VEP or INFO fields (default: "%(default)s")',
    )
    ...
    parser.add_argument(
        ...
        "--count_only",
        action="store_true",
        help="Output a count of matched entries only",
    )
    parser.add_argument(
        "-n", "--no_header", action="store_true", help="Don't write the VCF header"
    )
    ...
    with smart_open(args.output_file) as out_fh:
        ...
        for var in vcf_iter:
            allele_data = vep_vcf.get_all_allele_data(
                var=var, filter_fields=filter_fields
            )
            ...
            for allele_i in sorted(allele_data.keys()):
                passed += list(
                    filter(lambda x: eval_filter(filter_eval, x), allele_data[allele_i])
                )
